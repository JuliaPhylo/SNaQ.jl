...
"""
function perform_random_rNNI!(N::HybridNetwork, N_qdata::Vector{QuartetData}, Nprime_qdata::Vector{QuartetData}, param_map::Dict{Int,Int}, α::Real, probs::Vector{<:Real}=[0.25, 0.25, 0.25, 0.25])
    ...
    if r < probs[1]
        ...
        if length(valid_stuvs) == 0
            ...
            return perform_random_rNNI!(N, N_qdata, Nprime_qdata, α, [0, probs[2], probs[3], probs[4]] ./ p234)
        end
        s, t, u, v = sample(valid_stuvs)
        update =  perform_rNNI1!(N, s, t, u, v)
        apply_rNNI1_update!(N, N_qdata, Nprime_qdata, param_map, u)
        return update
    elseif r < probs[1] + probs[2]
        valid_stuvs = all_valid_rNNI2_nodes(N)
        if length(valid_stuvs) == 0
            p134 = probs[1] + probs[3] + probs[4]
            return perform_random_rNNI!(N, N_qdata, Nprime_qdata, α, [probs[1], 0, probs[3], probs[4]] ./ p134)
        end
        s, t, u, v = sample(valid_stuvs)
        update =  perform_rNNI2!(N, s, t, u, v)
        apply_rNNI2_update!(N, N_qdata, Nprime_qdata, param_map, s, t, u, v, α)
        return update
    elseif r < probs[1] + probs[2] + probs[3]
        valid_stuvs = all_valid_rNNI3_nodes(N)
        if length(valid_stuvs) == 0
            p124 = probs[1] + probs[2] + probs[4]
            return perform_random_rNNI!(N, N_qdata, Nprime_qdata, α, [probs[1], probs[2], 0, probs[4]] ./ p124)
        end
        s, t, u, v = sample(valid_stuvs)
        update = perform_rNNI3!(N, s, t, u, v)
        find_quartet_equations(N, Nprime_qdata) # update in-place
        return update
    else
        valid_stuvs = all_valid_rNNI4_nodes(N)
        if length(valid_stuvs) == 0
            p123 = probs[1] + probs[2] + probs[3]
            return perform_random_rNNI!(N, N_qdata, Nprime_qdata, α, [probs[1], probs[2], probs[3], 0] ./ p123)
        end
        s, t, u, v = sample(valid_stuvs)
        update = perform_rNNI4!(N, s, t, u, v)
        find_quartet_equations(N, Nprime_qdata) # update in-place
        return update
    end
end
