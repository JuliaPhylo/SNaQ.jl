...
@test size(bootnet)==(2,)
@test writenewick(bootnet[1], round=true) != writenewick(bootnet[2], round=true)
...
@testset "bootsnaq from quartet CF intervals with seed=0" begin
T=readnewick(joinpath(exdir,"startTree.txt"))
datf=DataFrame(CSV.File(joinpath(exdir,"tableCFCI.csv")); copycols=false)
originalstdout = stdout
redirect_stdout(devnull) # requires julia v1.6
bootnet = bootsnaq(T,datf,nrep=2,runs=1,seed=0,filename="",Nfail=2,
                   ftolAbs=1e-3,ftolRel=1e-3,xtolAbs=1e-4,xtolRel=1e-3,liktolAbs=0.01)
redirect_stdout(originalstdout)
@test size(bootnet)==(2,)
@test writenewick(bootnet[1], round=true) != writenewick(bootnet[2], round=true)
...
bootnet = bootsnaq(T,boottrees,nrep=2,runs=2,otherNet=net1,seed=1234,
                   prcnet=0.5,filename="",Nfail=500,ftolAbs=1e-12,ftolRel=1e-12)
redirect_stdout(originalstdout)
rmprocs(workers())
@test size(bootnet)==(2,)
